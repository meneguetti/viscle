...
class Viscle
{
    ...
    protected static function parseToJson(string $filename)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!self::isValidLine($rowExploded, count($nodesStack), !empty($nodesStack[0]['children']))) {
                continue;
            }
    ...
    protected static function fixUnclosedNodes($nodesStack)
    {
    ...
    protected static function isValidLine(array $rowExploded, int $stackQtyNodes, bool $hasChildren)
    {
        //not valid if
        // => less then 4 columns        
    ...
    protected static function filter(array $lifecycleDecoded)
    {
        ...
        foreach ($lifecycleDecoded as $index => $item) {
            ...
            //else marks this node has children
                ...
                $lifecycleDecoded[$index]['children'] = self::filter($item['children']);
            }
        }
